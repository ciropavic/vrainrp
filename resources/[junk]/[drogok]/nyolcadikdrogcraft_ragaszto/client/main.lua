...
function OpenFarmShop()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'Farm_shop', {
		title    = _U('dealer_title'),
		align    = 'right',
		elements = elements
	}, function(data, menu)
		TriggerServerEvent('vrain_nyolcadikdrogcraft:sellFarm', data.current.name, data.current.value)
...
function OpenBuyLicenseMenu(licenseName)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'license_shop', {
		...
		if data.current.value ~= 'no' then
			ESX.TriggerServerCallback('vrain_nyolcadikdrogcraft:buyLicense', function(boughtLicense)
