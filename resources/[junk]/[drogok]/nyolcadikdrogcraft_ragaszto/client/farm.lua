...
function Processfarm(xfarm)
  ...
  TriggerServerEvent('vrain_nyolcadikdrogcraft:processfarm')
	if(xfarm <= 3) then
		xfarm = 0
	end
  local timeLeft = (Config.Delays.FarmProcessing * xfarm) / 1000
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.FarmProcessing.coords, false) > 4 then
			...
			TriggerServerEvent('vrain_nyolcadikdrogcraft:cancelProcessing')
			TriggerServerEvent('vrain_nyolcadikdrogcraft:outofbound')
			break
		end
	end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				ESX.TriggerServerCallback('vrain_nyolcadikdrogcraft:canPickUp', function(canPickUp)
					if canPickUp then
						exports.ox_inventory:Progress({
							duration = 60000,
							label = 'Alapanyag begyűjtése...',
							useWhileDead = false,
							canCancel = false,
							disable = {
								move = true,
						...
						TriggerServerEvent('vrain_nyolcadikdrogcraft:pickedUpfarm')
					else
