			...
			if distance < v.radius + 2 then
				...
				if distance < v.radius then
					...
					if not Config.UseOkokTextUI and not isCraftOpen then
						ESX.ShowHelpNotification('Nyomd meg az ~INPUT_CONTEXT~ gombot, hogy megnyisd a barkácsolási panelt')
					end
					if IsControlJustReleased(0, Config.Key) then
						if GetVehiclePedIsUsing(PlayerPedId()) == 0 then
							...
							exports['vrain_notify']:showNotify('Nem barkácsolhatsz miközben járműben vagy.', 'error', 8000)
						end
					end
				end
