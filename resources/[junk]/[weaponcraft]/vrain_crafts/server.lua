...
local Webhook = 'https://discord.com/api/webhooks/963122653986717718/0lOsuSfI9i1uS9VPLTIb1Ye8foYSsSjXarkIpQ-c0ywEVo_7JWjKrF_UgpGfEdhXWiDk'
local sessions = {}
...
AddEventHandler('okokCrafting:craftItemDeath',function(queueClient)
	...
	if sessions[source] then
		if sessions[source].stoppedCraft then
			...
			TriggerClientEvent('vrain_notify:showNotification', source, 'Mivel barkácsolás közben haltál meg, visszakaptad az alapanyagokat.', 'warn', 10000)
		...
		TriggerClientEvent('vrain_notify:showNotification', source, 'Hiba történt. (No session)', 'error', 8000)
	end
...
AddEventHandler('okokCrafting:craftItemFinished', function(item, crafts, itemName, isItem)
	...
	if sessions[source] then
		...
		if sessions[source].last+500 >= timeToCraft then
			...
			TriggerClientEvent('vrain_notify:showNotification', source, 'Anti-cheat védelem', 'error', 8000)
		end
	else
		if Webhook ~= '' then
			local identifierlist = ExtractIdentifiers(xPlayer.source)
...
ESX.RegisterServerCallback("okokCrafting:CanCraftItem", function(source, cb, itemID, recipe, itemName, amount)
	...
	if canCraft then
		if xPlayer.canCarryItem(itemID, amount) then
			...
			cb(true)
			TriggerClientEvent('vrain_notify:showNotification', source, itemName[itemID]..' elhelyezve a várólistára.', 'success', 8000)
			if Webhook ~= '' then
				local identifierlist = ExtractIdentifiers(xPlayer.source)
				local data = {
					playerid = xPlayer.source,
			...
			cb(false)
			TriggerClientEvent('vrain_notify:showNotification', source, 'Nem fér el nálad: '..itemName[itemID], 'error', 8000)
		end
	else
		cb(false)
		TriggerClientEvent('vrain_notify:showNotification', source, 'Nem barkácsolhatod le: '..itemName[itemID], 'error', 8000)
	end
end)
