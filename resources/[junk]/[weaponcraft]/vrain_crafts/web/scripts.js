...
window.addEventListener('message', function(event) {
				...
				for(var i1 = 0; i1 < craft[i].job.length; i1++){
					...
					if(!myEle) {
						$(document).on('click', "#"+itemId+"wb_"+event.data.wb, function() {
							...
							sound.volume = 0.3;
							sound.play();
							$.post('http://vrain_crafts/action', JSON.stringify({
								action: "craft",
								item: id,
								crafts: craft,
								itemName: itemName,
	...
	case 'openSideCraft':
		...
		if(canCraft) {
			$('#craft-button-div').html(`
				<button type="button" id="craft-button" data-item="${event.data.itemNameID}" data-recipe="${event.data.recipe}" data-amount="${event.data.itemAmount}" onclick="craft(this)" class="btn btn-blue flex-grow-1 mt-2" style="border-radius: 10px; flex-basis: 100%; width: 100%; font-weight: 600;"><i class="fas fa-pencil-ruler"></i> ${event.data.itemName} barkácsolása</button>
			`);
			$('#craft-button').fadeIn();
		} else {
	...
	case 'ShowCraftCount':
		...
		$('#crafting-text').html(`Készítés..`);
		$('#item-timer').html(`${time} mp.`);
		if(time == lastTime) {
	...
	case 'CompleteCraftCount':
		...
		$('#crafting-text').html(`Sikeres barkácsolás`);
		$('#item-timer').html(`0 mp.`);
	...
	case 'FailedCraftCount':
		...
		$('#crafting-text').html(`Nem sikerült a barkácsolás`);
		$('#item-timer').html(`0 mp.`);
