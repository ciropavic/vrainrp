...
function Processfarm(xfarm)
  ...
  TriggerServerEvent('vrain_masodikcraft:processfarm')
	if(xfarm <= 3) then
		xfarm = 0
	end
  local timeLeft = (Config.Delays.FarmProcessing * xfarm) / 1000
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.FarmProcessing.coords, false) > 4 then
			...
			TriggerServerEvent('vrain_masodikcraft:cancelProcessing')
			TriggerServerEvent('vrain_masodikcraft:outofbound')
			break
		end
	end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				ESX.TriggerServerCallback('vrain_masodikcraft:canPickUp', function(canPickUp)
					if canPickUp then
						...
						TriggerServerEvent('vrain_masodikcraft:pickedUpfarm')
					else
