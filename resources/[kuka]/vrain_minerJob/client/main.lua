...
local blips = {
	{title="Bánya", colour=0, id=357, x=-593.95855712891, y=2091.75390625, z=131.71856689453},
  }
...
Citizen.CreateThread(function()
    ...
    for _, info in pairs(blips) do
	  ...
	  local zoneblip = AddBlipForRadius(info.x, info.y, info.z, 300.0)
...
AddEventHandler("showReward:minerJob", function(completReward)
	exports['vrain_notify']:showNotify('Kaptál a munkádért' ..CompletReward.. '$-t.', 'success', 4000)
end)
		...
		if delivery_point_distance < 500.0 then
			...
			if delivery_point_distance < 1.5 then
				if isMining == false then
					DisplayHelpText("Nyomd meg a(z) ~INPUT_CONTEXT~ betüt, hogy elkezdj ~r~bányászni.")
					...
					if(IsControlJustReleased(1, 38))then
						if pickaxeCounter > 0 then
							...
							exports['vrain_notify']:showNotify('Nincsen csákányod, kérlek vásárolj egyet a bánya előtt álló főnöktől', 'error', 4000)
						end
...
AddEventHandler("pickaxeBroken:minerJob", function()
	exports['vrain_notify']:showNotify('Eltört a csákányod, használj egy másikat', 'warn', 4000)
end)
