	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerClientEvent('vrain_notify:showNotification', source, _U('have_withdrawn', ESX.Math.GroupDigits(amount)), 'success', 10000)
			else
				TriggerClientEvent('vrain_notify:showNotification', source, _U('invalid_amount'), 'error', 10000)
			end
		end)
	else
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				...
				TriggerClientEvent('vrain_notify:showNotification', source, _U('have_deposited', ESX.Math.GroupDigits(amount)), 'success', 10000)
	...
	if xPlayer.job.name == society then
		if amount and amount > 0 and account.money >= amount then
			...
			function(rowsChanged)
				TriggerClientEvent('vrain_notify:showNotification', source, __U('you_have', ESX.Math.GroupDigits(amount)), 'info', 10000)
			end)
		else
			TriggerClientEvent('vrain_notify:showNotification', source, _U('invalid_amount'), 'error', 10000)
		end
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if isPlayerBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				TriggerClientEvent('vrain_notify:showNotification', source, _U('you_have_been_hired', job), 'info', 10000)
			elseif type == 'promote' then
				TriggerClientEvent('vrain_notify:showNotification', source, _U('you_have_been_promoted'), 'info', 10000)
			elseif type == 'fire' then
				TriggerClientEvent('vrain_notify:showNotification', source, _U('you_have_been_fired', xTarget.getJob().label), 'info', 10000)
			end
...
function isPlayerBoss(playerId, job)
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'boss' or xPlayer.job.grade_name == 'helyettes' or xPlayer.job.grade_name == 'ezredes' or xPlayer.job.grade_name == 'igazgato' then
		return true
	else
...
function WashMoneyCRON(d, h, m)
	MySQL.query('SELECT * FROM society_moneywash', function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				TriggerClientEvent('vrain_notify:showNotification', source, _U('you_have_laundered', ESX.Math.GroupDigits(result[i].amount)), 'info', 10000)
			end
