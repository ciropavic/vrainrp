...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords)
	...
	function self.setMeta(key, value, force)
		if (not key) then 
			return false;
		end 
		...
		if (self.__metadata[key] and not force) then 
			return false;
		end 

		triggerEvent("onPlayerDataChange", self.source, key, self.__metadata[key], value);
		self.__metadata[key] = value;
	end 

	function self.setLocalMeta()
		if (not key) then 
			return false;
		end 
		...
		self.__metadata[key] = value;
		TriggerClientEvent("es_extended:setLocalMeta", self.source, key, value);
	end 
