...
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `admin`, `adminjail`'
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	local admin = json.decode(result.admin);
	if (admin and admin.level > 0) then 
		exports.vAdmin:setupPlayerAsAdmin(playerId, admin.level, admin.name);
	end 

	local jail = json.decode(result.adminjail);
	if (jail) then 
		exports.vAdmin:sendPlayerToJail(playerId, jail.left, jail.jail_type, jail.admin.id, jail.admin.name, jail.reason);
	end 
...
AddEventHandler('chatMessage', function(playerId, author, message)
	...
	if message:sub(1, 1) == '/' and playerId > 0 then
		...
		template = '<div class="chat-message system"><i class="fas fa-cog"></i> <b><span style="color: #df7b00">SYSTEM</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{1}</span></b><div style="margin-top: 5px; font-weight: 300;"><b>{0}</b> helytelen parancs!</div></div>',
