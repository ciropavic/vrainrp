...
for k,v in pairs(Config.job) do
	if v.ambulance and xPlayer.job.name == 'ambulance' then
		if args[1] ~= nil then
			...
			TriggerClientEvent('vrain_notify:showNotification', source, 'Nem található ilyen játékos! (ID)', 'error', 8000)
		end				
	...
	elseif v.mechanic and xPlayer.job.name == 'mechanic' then
		...
		TriggerClientEvent('vrain_notify:showNotification', source, 'Nem vagy a megfelelő frakcióban!', 'error', 8000)
	end
