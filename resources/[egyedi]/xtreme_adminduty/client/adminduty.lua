...
AddEventHandler('asdasd_adminduty_setadminonduty', function(adminondutys)
...
while not ESX do Citizen.Wait(50) end
    ...
    Wait(1000)
        ...
        if NetworkIsPlayerActive(player)  and GetPlayerPed(player) ~= GetPlayerPed(-1) then
			...
			gamertag = CreateMpGamerTag(GetPlayerPed(player), GetPlayerName(player) ..' | '..GetPlayerServerId(player), false, false, '', 0)
...
DrawMarker(9, x,y,z+Xtreme.xcoords, 0.0, 0.0, 0.0, 90.0, 90.0, 0.0, Xtreme.size, Xtreme.size, 0.0, 255, 255, 255, 244, false, false, 2, true, "adminlogo", "logo", false)
end
...
Citizen.CreateThread(function()
	while true do
		...
		if godmode then
			...
			SetPlayerFallDistance(playerped, 20.0)
		end	
	      ...
	      sleep = 1000
				for i, value in ipairs(adminonduty) do
					for i2, value2 in ipairs(nearplayersdata) do
						...
						drawmarker(x,y,z)
						ASDASD3DTEXT(targetPedCords+vector3(0,0,Xtreme.xcoordstext), GetPlayerName(value2).." "..Xtreme.admingroups[value.playergroup], 0.4)					
					end
				end
            end
        end
...
while true do
sleep = 5
	if IsControlJustReleased(0, Xtreme.menukey) then
		ESX.TriggerServerCallback('asdasd_checkhaspermission', function(hasperm)
		if hasperm then
			openUI()
			else
			sleep = 5000
...
ESX.TriggerServerCallback('xtreme_admindutygetinfo', function(data)	
...
RegisterNUICallback('logoon', function()
TriggerServerEvent('xtreme_adminduty_logo')
end)

RegisterNUICallback('dutyoff', function()
TriggerServerEvent('xtreme_adminduty', dutytime)
end)

RegisterNUICallback('dutyon', function()
TriggerServerEvent('xtreme_adminduty')
end)

RegisterNUICallback('jumpon', function()
if adminduty then
bigjump = true
else
...
RegisterNUICallback('jumpoff', function()
if adminduty then
bigjump = false
else
ESX.ShowNotification('~r~Nem vagy adminszolgálatban!')
...
RegisterNUICallback('godmodeoff', function()
if adminduty then
godmode = false
else
ESX.ShowNotification('~r~Nem vagy adminszolgálatban!')
...
RegisterNUICallback('godmodeon', function()
if adminduty then
godmode = true
else
ESX.ShowNotification('~r~Nem vagy adminszolgálatban!')
...
RegisterNUICallback('invisibleoff', function()
if adminduty then
invisible = false
else
ESX.ShowNotification('~r~Nem vagy adminszolgálatban!')
...
RegisterNUICallback('invisibleon', function()
if adminduty then
invisible = true
else
ESX.ShowNotification('~r~Nem vagy adminszolgálatban!')
...
RegisterNUICallback('speedoff', function()
if adminduty then
speed = false
else
ESX.ShowNotification('~r~Nem vagy adminszolgálatban!')
...
RegisterNUICallback('speedon', function()
if adminduty then
speed = true
else
ESX.ShowNotification('~r~Nem vagy adminszolgálatban!')
...
RegisterNUICallback('flyoff', function()
if adminduty then
fly = false
else
ESX.ShowNotification('~r~Nem vagy adminszolgálatban!')
...
RegisterNUICallback('flyon', function()
if adminduty then
fly = true
else
ESX.ShowNotification('~r~Nem vagy adminszolgálatban!')
