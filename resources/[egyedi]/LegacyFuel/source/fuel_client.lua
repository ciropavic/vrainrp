...
AddEventHandler('fuel:requestJerryCanPurchase', function()
	...
	if currentCash >= Config.JerryCanCost then
		...
		if not HasPedGotWeapon(ped, 883325847) then
			exports['vrain_notify']:showNotify(Config.Strings.PurchaseJerryCan, 'success', 4000)
			...
			if Config.UseESX then
				...
				if refillCost > 0 then
					if currentCash >= refillCost then
						exports['vrain_notify']:showNotify(Config.Strings.RefillJerryCan .. "~g~$" .. refillCost, 'info', 4000)
						TriggerServerEvent('fuel:pay', refillCost)
						SetPedAmmo(ped, 883325847, 4500)
					else
						exports['vrain_notify']:showNotify(Config.Strings.NotEnoughCashJerryCan, 'error', 4000)
					end
				else
					exports['vrain_notify']:showNotify(Config.Strings.JerryCanFull, 'error', 4000)
				end
			else
				exports['vrain_notify']:showNotify(Config.Strings.RefillJerryCan, 'success', 4000)
				SetPedAmmo(ped, 883325847, 4500)
			end
		end
	else
		exports['vrain_notify']:showNotify(Config.Strings.NotEnoughCash, 'error', 4000)
	end
end)
