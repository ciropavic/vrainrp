    ...
    if (event.data.type == "updateVehicleInfos") {
        ...
        $('#contentVehicle').append(`
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Irányíthatóság</span>
                        <div class="bar">
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Maximum sebesség</span>
                        <div class="bar">
                    ...
                    <div class="column spacebetween info">
                        <span class="title">Gyorsulás</span>
                        <div class="bar">
                    ...
                    <div class="row" id="actionButton">
                        <a class="buyButton" style=" color:#FC1259;" onclick="openModalMenu()"> Vásárlás </a>
                    </div>
                `);

                if (isEnableTestDrive) {
                    $('#actionButton').append(`
                        <a class="tdButtton" onclick="testDrive()"> Teszt vezetés </a>
                    `);
                }
    }
...
function menuVehicle(event) {
    ...
    $.post('https://vrain_autobolt/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('https://vrain_autobolt/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));    
...
function openModalMenu(){    
    ...
    $('#closemenu').append(`
        ...
        <div class="modal-content">
            <p class="title">Vásárlás megerősítés:</p>
        ...
        <div class="modal-footer">
            <div class="modal-price">
                <p class='price sale'> `+ vehicleSelected.sale +`$</p>                
            </div>
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('https://vrain_autobolt/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $.post('https://vrain_autobolt/Close');
            $('body').css("display","none");
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('https://vrain_autobolt/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('https://vrain_autobolt/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('https://vrain_autobolt/rotate', JSON.stringify({key: _key}))
    }
    ...
    switch(event.which) {        
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                $.post("https://vrain_autobolt/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));
...
function setVehicleColorRGB(R,G,B){
    ...
    if (selectedColor == 'primary'){
        $.post("https://vrain_autobolt/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
    } else {
        $.post("https://vrain_autobolt/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
    }
}
