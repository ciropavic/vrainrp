			...
			if v.modType then
				...
				elseif v.modType == 17 then -- TURBO
					...
					if currentMods[k] then
						...
						_label = 'Turbo - <span style="color:green;">$' .. math.floor(vehiclePrice * v.price[1]) .. ' </span>'
					end
					table.insert(elements, {label = _label, modType = k, modNum = true})
				else
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'mechanic' or PlayerData.job.name == 'medellin') or not Config.IsMechanicJobOnly then
