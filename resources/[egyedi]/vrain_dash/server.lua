...
ESX.RegisterServerCallback("requestPlayerDatas", function(source, cb)
	...
	table.insert(result, {
		title = "Admin rang",
		value = (exports.vAdmin:getPlayerAdminTitle(source) or "Játékos") .. ((admin and admin.level) and " (" .. admin.level .. ")" or "")
	})
	...
	table.insert(
		result,
		{ title = "Fizetés", value = xPlayer.job and xPlayer.job.grade_salary .. "$" or "Nincs 😢", color = "#7946c5" }
	)
	...
	table.insert(result, { title = "Játszott idő", value = secondsToClock(playedTime), color = "#7946c5" })
	table.insert(result, { title = "Online Idő", value = secondsToClock(currentSessionTime), color = "#7946c5" })
