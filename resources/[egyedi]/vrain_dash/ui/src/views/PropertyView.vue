  ...
  <div>
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <h5
          v-if="search !== '' && filteredVehicles.length <= 0"
          class="text-light text-center mt-5"
        >
        ...
        >
          ...
          <td class="vehicle-name fw-bold">
            {{ vehicle.label ?? "Ismeretlen" }}
          </td>
          <td
            :style="{
          ...
          <td :style="{ color: getColor(vehicle.fuel) }" class="fuel">
            {{ vehicle.fuel ?? 0 }}%
          </td>
        </tr>
      </tbody>
    ...
    <h6
      v-if="vehicles.length >= 0 && currentVehicle !== undefined"
      class="fw-bold"
    >
      <div class="d-flex">
        <div>
          ...
          <ul class="current-vehicle">
            <li>
              Tárolva:
              <span
                :style="{ color: currentVehicle.stored ? 'green' : 'red' }"
                >{{ currentVehicle.stored ? "Igen" : "Nem" }}</span
              >
            </li>
            <li>Rendszám: {{ currentVehicle.plate }}</li>
            <li>
              Szín 1: {{ currentVehicle.vehicle.color1 }} | Szín 2:
              {{ currentVehicle.vehicle.color2 }}
            </li>
            <li>
            ...
            <li>
              Üzemanyag:
              <span :style="{ color: getColor(currentVehicle.fuel) }"
                >{{ currentVehicle.fuel }}%</span
              >
            </li>
            <li>
              Motor állapot:
              <span
                :style="{
                  color: getColor(currentVehicle.vehicle.engineHealth / 10),
                }"
              >
            ...
            <li>
              ...
              <span
                :style="{
                  color: getColor(currentVehicle.vehicle.bodyHealth / 10),
                }"
              >
            ...
            <li>
              ...
              <span
                :style="{
                  color: getColor(currentVehicle.vehicle.tankHealth / 10),
                }"
        ...
        <div class="ms-3 mt-4">
          ...
          <ul>
            <li
              v-for="(label, name) in performanceMods"
              :key="name"
              v-html="getMod(name)"
            ></li>
          </ul>
        ...
        <div class="optical-tunings ms-3 mt-4">
          ...
          <ul>
            <li
              v-for="(label, name) in opticalMods"
              :key="name"
              v-html="getMod(name)"
            ></li>
          </ul>
  ...
  methods: {
    ...
    getMod(name) {
      ...
      if (!value) value = `<span class="text-danger">Nincs</span>`;
        ...
        value =
          value <= 0
            ? `<span class="text-danger">Nincs</span>`
            : `<span class="text-primary">${value}</span>`;

      const label =
        this.performanceMods[name] ?? this.opticalMods[name] ?? "Ismeretlen";
      return `${label}: ${value}`;
    },
  },
  async created() {
    ...
    const response = await fetch(
      `https://${GetParentResourceName()}/requestData`,
      {
        method: "POST",
        body: JSON.stringify({
          name: "property",
