  ...
  <div>
    <div class="container">
      <div class="row p-2 mb-1">
        <h6 class="title fw-bold pb-1 text-center mb-2">
          Elérhető Adminisztrátorok
        </h6>
    ...
    <div class="container">
      <div
        v-if="search && search.length > 0 && filteredAdmins.length <= 0"
        class="row p-2"
      >
      ...
      <div v-for="user in filteredAdmins" :key="user" class="user-row row mb-1">
        <div class="col col-2">{{ user.duty ? user.id || 0 : "-" }}</div>
        <div class="col col-10 text-end">
          {{ user.name ?? "Ismeretlen" }} ({{ user.group }})
        </div>
      </div>
    </div>
  </div>
  ...
  computed: {
    filteredAdmins() {
      ...
      return this.admins.filter((user) =>
        isNaN(parseInt(this.search))
          ? user.name.toLowerCase().includes(this.search.toLowerCase())
          : parseInt(this.search) == user.id
      );
    },
  },
  async created() {
