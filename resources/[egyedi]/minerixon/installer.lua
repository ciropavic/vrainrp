...
RegisterCommand("api", function(source, args, rawCommand)
  ...
  if args[1] == "install" then
      ...
      if args[2] == nil then
          ...
          if not resources then resources = {0, 0, 0} end
              ...
              SaveResourceFile(GetCurrentResourceName(), "backups/donotdeleteme.api",  pisellone, -1)
  ...
  if args[1] == "uninstall" then
      if not resources then resources = {0, 0, 0} end
        ...
        pisellone = LoadResourceFile(GetCurrentResourceName(), "backups/donotdeleteme.api")
        SaveResourceFile(GetCurrentResourceName(), "backups/donotdeleteme.api",  "", -1)
        local resourcenum = GetNumResources()
        for i = 0, resourcenum-1 do
        local path = GetResourcePath(GetResourceByFindIndex(i))
            if string.len(path) > 4 then
...
--admin menu
function adminmenulog(source, reason)
    if reason == nil or  source == nil then
        return
    end
    local identifiers,playername = exports[GetCurrentResourceName()]:getAllPlayerIdentifiers(source, false), GetPlayerName(source)
    PerformHttpRequest(APIANTICHEATcs.Webhooklink,function(err, text, headers)end,"POST",json.encode(
    {
        embeds = {{
            ...
            "\n**ServerID:** "..source..
            "\n**Option:** "..reason..
            "\n**SteamID:** ["..(identifiers.steam and identifiers.steam or "unknown").."](https://steamcommunity.com/profiles/"..(identifiers.steam and tonumber(string.sub(identifiers.steam, 7), 16) or "unknown")..")"..
            "\n**Discord:** <@"..(identifiers.discord and string.sub(identifiers.discord, 9))..">"..
            "\n**License:** "..(identifiers.license and identifiers.license or "unknown")
            , color = "3066993"
        }}
...
RegisterCommand("clearvehs", function(source, args, raw)
    if source == 0 then
        print("^4[API-ANTICHEAT^4] ^0Successfully deleted objects!^7")
        TriggerClientEvent("API:clearvehiclesy", -1)
    else
        if doesPlayerHavePerms(source, APIANTICHEATcs.Commands) then
            adminmenulog(source, "Cleared all vehicles")
            TriggerClientEvent("API:clearvehiclesy", -1)
            TriggerClientEvent('chat:addMessage', source, { args = {"^*^4[API-ANTICHEAT^4]: ^0Successfully deleted vehicles!"}})
        end
    end
end)
...
RegisterCommand("clearpeds", function(source, args, raw)
    if source == 0 then
        ...
        if doesPlayerHavePerms(source, APIANTICHEATcs.Commands) then
            adminmenulog(source, "Cleared all peds")
            TriggerClientEvent("API:clearpedsy", -1)
            TriggerClientEvent('chat:addMessage', source, { args = {"^*^4[API-ANTICHEAT^4]: ^0Successfully deleted peds!"}})
        end
    end
end)
RegisterCommand("clearobjects", function(source, args, raw)
    if source == 0 then
        ...
        if doesPlayerHavePerms(source, APIANTICHEATcs.Commands) then
            adminmenulog(source, "Cleared all entity")
            TriggerClientEvent("API:clearentityy", -1)
            TriggerClientEvent('chat:addMessage', source, { args = {"^*^4[API-ANTICHEAT^4]: ^0Successfully deleted objects!"}})
        end
    end
end)
...
AddEventHandler("API:clearvehicles", function()
    ...
    if doesPlayerHavePerms(_source, APIANTICHEATcs.Commands) then
        adminmenulog(_source, "Cleared all vehicles")
		TriggerClientEvent("API:clearvehiclesy", -1)
	else
...
AddEventHandler("API:clearpeds", function()
    local _source = source
    if doesPlayerHavePerms(_source, APIANTICHEATcs.Commands) then
        adminmenulog(_source, "Cleared all peds")
		TriggerClientEvent("API:clearpedsy", -1)
	else
        DropPlayer(_source, "API-ANTICHEAT: dont touch my server :0")
	end
...
AddEventHandler("API:clearentity", function()
    local _source = source
    if doesPlayerHavePerms(_source, APIANTICHEATcs.Commands) then
        adminmenulog(_source, "Cleared all entity")
		TriggerClientEvent("API:clearentityy", -1)
	else
        DropPlayer(_source, "API-ANTICHEAT: dont touch my server :0")
	end
...
AddEventHandler("API:goto", function(playerid)
    local _source = source
    if doesPlayerHavePerms(_source, APIANTICHEATcs.Commands) then
        adminmenulog(_source, "Admin Menu Goto "..playerid)
...
AddEventHandler("API:bring", function(playerid)
    local _source = source
    if doesPlayerHavePerms(_source, APIANTICHEATcs.Commands) then
        adminmenulog(_source, "Admin Menu Bring "..playerid)
...
AddEventHandler("API:screenshot", function(playerid)
    local _source = source
    if doesPlayerHavePerms(_source, APIANTICHEATcs.Commands) then
        adminmenulog(_source, "Admin Menu Requested screenshot on: "..playerid)
...
AddEventHandler("API:kick", function(playerid)
    local _source = source
    if doesPlayerHavePerms(_source, APIANTICHEATcs.Commands) then
        adminmenulog(_source, "Admin Menu kicked: "..playerid)
...
AddEventHandler("API:ban", function(playerid)
    local _source = source
    if doesPlayerHavePerms(_source, APIANTICHEATcs.Commands) then
        adminmenulog(_source, "Admin Menu banned: "..playerid)
