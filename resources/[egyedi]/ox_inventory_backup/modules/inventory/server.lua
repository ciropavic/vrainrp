...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if item then
		if inv then
			exports.vCore:iprint("kurva?", inv)
...
function Inventory.RemoveItem(inv, item, count, metadata, slot)
	...
	if item and count > 0 then
		...
		if (metadata ~= nil and (metadata:gsub(" ", "")) ~= "") then
...
local function dropItem(source, data)
	...
	exports['vrain_logs']:createLog({
		EmbedMessage = '**'..data.count..'x '..toData.name..'** tárgy, átadásra került **'..playerInventory.label..'** leltárából **'..dropId..'** leltárába',
		color = '#000000',
		channel = 'AntiCheat',
					...
					if toData and ((toData.name ~= fromData.name) or not toData.stack or (not table.matches(toData.metadata, fromData.metadata))) then
						...
						if not sameInventory then
							if fromInventory.type == 'container' or (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
								...
								exports['vrain_logs']:createLog({
									EmbedMessage = '**'..fromData.count..'x '..fromData.name..'** **'..fromInventory.owner and fromInventory.label or fromInventory.id..'** raktárából, átadásra került **'..toInventory.owner and toInventory.label or toInventory.id..' raktárába, '..toData.count..'x '..toData.name..'** tárgy',
									color = '#000000',
									channel = 'AntiCheat',
									screenshot = false -- Set this to true if you want to screenshot the players screen
								})
					...
					elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
						...
						if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
							...
							if not sameInventory then
								...
								exports['vrain_logs']:createLog({
									EmbedMessage = '**'..data.count..'x '..fromData.name..'** **'..fromInventory.owner and fromInventory.label or fromInventory.id..'** átadásra került **'..toInventory.owner and toInventory.label or toInventory.id..' asd',
									color = '#000000',
									channel = 'AntiCheat',
									screenshot = false -- Set this to true if you want to screenshot the players screen
								})
					...
					elseif data.count <= fromData.count then
						...
						if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
							if not sameInventory then
								...
								exports['vrain_logs']:createLog({
									EmbedMessage = '**'..data.count..'x '..fromData.name..'** tárgy **'..fromInventory.label..'** leltárából, átadva **'..toInventory.label..'** leltárába',
									color = '#000000',
									channel = 'AntiCheat',
									screenshot = false -- Set this to true if you want to screenshot the players screen
								})
	...
	if toInventory.type == 'player' then
		...
		if not toInventory.open and Inventory.CanCarryItem(toInventory, item, count, data.metadata) then
			if data and data.count >= count then
				...
				exports['vrain_logs']:createLog({
					EmbedMessage = fromInventory.label..' átadott '..toInventory.label..' **'..count..' '..data.name..'**',
					color = '#000000',
					channel = 'AntiCheat',
					screenshot = false -- Set this to true if you want to screenshot the players screen
				})
...
-- lib.addCommand(false, {'additem', 'giveitem'}, function(source, args)
...
-- 		exports['vrain_logs']:createLog({
-- 			EmbedMessage = '*ADMIN* **'..source.label..'** átadott **'..args.count..'x** **'..args.item.name..'** **'..inventory.label..'** játékosnak',
-- 			color = '#000000',
-- 			channel = 'AntiCheat',
-- 			screenshot = false -- Set this to true if you want to screenshot the players screen
-- 		})
...
-- lib.addCommand(false, 'removeitem', function(source, args)
-- 	args.item = Items(args.item)
-- 	exports.vCore:iprint(args);
-- 	if args.item and args.count > 0 then
...
-- 		)
-- 		exports['vrain_logs']:createLog({
-- 			EmbedMessage = '*ADMIN* **'..source.label..'** elvett **'..args.count..'x** **'..args.item.name..'** **'..inventory.label..'** játékostól',
-- 			color = '#000000',
-- 			channel = 'AntiCheat',
-- 			screenshot = false -- Set this to true if you want to screenshot the players screen
-- 		})
...
-- lib.addCommand('group.foadmin', 'setitem', function(source, args)
-- 	args.item = Items(args.item)
-- 	if args.item and args.count >= 0 then
...
-- 		)
-- 		exports['vrain_logs']:createLog({
-- 			EmbedMessage = '*ADMIN* **'..source.label..'** beállította **'..inventory.label..'** játékosnak, **'..args.item.name..'** tárgyszámát **'..args.count..'** mennyiségre',
-- 			color = '#000000',
-- 			channel = 'AntiCheat',
-- 			screenshot = false -- Set this to true if you want to screenshot the players screen
-- 		})
...
-- lib.addCommand('group.foadmin', 'takeinv', function(source, args)
...
-- lib.addCommand('group.foadmin', 'clearinv', function(source, args)
...
-- lib.addCommand('group.adminseged', 'viewinv', function(source, args)
...
exports.vAdmin:addCommand(
	"giveitem", 
	{
		...
		args = {
			{ type = "player" }, 
			{ type = "string", name = 'Item ID' }, 
			{ type = "number", name = "Mennyiség" }, 
			{ type = "string", name = "Metatype", optional = true, default = "" }, 
		},
		alias = { "additem" },
	}, 
	function(player, targetPlayer, itemId, count, metadata)
		if (count < 1) then 
			return exports.vChat:outputChatBox(player.__netId, "Autista vagy hogy kevesebbet akarsz adni mint egyet?");
		end 

		local item = Items(itemId);
		if (not item) then 
			return exports.vChat:outputChatBox(player.__netId, "Idk lehet nem letezik az itemxd");
		end 

		Inventory.AddItem(targetPlayer.__netId, item.name, count, metadata);
		local targetInventory = Inventories[targetPlayer.__netId];
		local adminInventory = Inventories[player.__netId] or {label = 'console', owner = 'console'};

		exports['vrain_logs']:createLog({
			EmbedMessage = '*ADMIN* **'..exports.vAdmin:getPlayerAdminName(player.__netId)..'** átadott **'..count..'x** **'..itemId..'** **'..targetInventory.label..'** játékosnak',
			color = '#000000',
			channel = 'AntiCheat',
			screenshot = false -- Set this to true if you want to screenshot the players screen
		})
...
exports.vAdmin:addCommand(
	...
	function(player, targetPlayer, itemId, count, metadata)
		...
		Inventory.RemoveItem(targetPlayer.__netId, item.name, count, (metadata and metadata:gsub(" ", "") ~= "") and metadata or nil, nil);
		local targetInventory = Inventories[targetPlayer.__netId];
		local adminInventory = Inventories[player.__netId] or {label = 'console', owner = 'console'};
...
exports.vAdmin:addCommand(
	...
	function(player, targetPlayer, itemId, count, metadata)
		...
		Inventory.SetItem(targetPlayer.__netId, item.name, count, (metadata and metadata:gsub(" ", "") ~= "") and metadata or nil, nil);
		local targetInventory = Inventories[targetPlayer.__netId];
		local adminInventory = Inventories[player.__netId] or {label = 'console', owner = 'console'};

		exports['vrain_logs']:createLog({
			EmbedMessage = '*ADMIN* **'..exports.vAdmin:getPlayerAdminName(player.__netId)..'** beállította **'..targetInventory.label..'** játékosnak, **'..item.name..'** tárgyszámát **'..count..'** mennyiségre',
			color = '#000000',
			channel = 'AntiCheat',
			screenshot = false -- Set this to true if you want to screenshot the players screen
		});
...
exports.vAdmin:addCommand(
	...
	function(player, targetPlayer)
		Inventory.Confiscate(targetPlayer.__netId);
		local targetInventory = Inventories[targetPlayer.__netId];

		exports['vrain_logs']:createLog({
			EmbedMessage = '*ADMIN* **'..exports.vAdmin:getPlayerAdminName(player.__netId)..'** elvette **'..targetInventory.label..'** játékos tárgyait.',
			color = '#000000',
			channel = 'AntiCheat',
			screenshot = false -- Set this to true if you want to screenshot the players screen
		});
...
exports.vAdmin:addCommand(
	...
	function(player, targetPlayer)
		Inventory.Return(targetPlayer.__netId);
		local targetInventory = Inventories[targetPlayer.__netId];

		exports['vrain_logs']:createLog({
			EmbedMessage = '*ADMIN* **'..exports.vAdmin:getPlayerAdminName(player.__netId)..'** visszaadta **'..targetInventory.label..'** játékos tárgyait.',
			color = '#000000',
			channel = 'AntiCheat',
			screenshot = false -- Set this to true if you want to screenshot the players screen
		});
...
exports.vAdmin:addCommand(
	...
	function(player, targetPlayer)
		Inventory.Clear(targetPlayer.__netId);
		local targetInventory = Inventories[targetPlayer.__netId];

		exports['vrain_logs']:createLog({
			EmbedMessage = '*ADMIN* **'..exports.vAdmin:getPlayerAdminName(player.__netId)..'** törölte **'..targetInventory.label..'** játékos tárgyait.',
			color = '#000000',
			channel = 'AntiCheat',
			screenshot = false -- Set this to true if you want to screenshot the players screen
		});
...
exports.vAdmin:addCommand(
	"saveinv", 
	{
		required = { admin = 5, off_admin = 5 },
		args = {
			{ type = "number", values = { 0, 1 } }
		},
	}, 
	function(player, num)
		saveInventories(num == 1);
	end 
);
...
exports.vAdmin:addCommand(
	...
	function(player, targetPlayer)
		if (player == targetPlayer) then 
			return exports.vChat:outputChatBox(player.__netId, "Miert akarnad megnezni a sajat inventoryd lol");
		end 

		local inventory = Inventories[targetPlayer.__netId] or Inventories[tonumber(targetPlayer.__netId)];
		TriggerClientEvent('ox_inventory:viewInventory', player.__netId, inventory);
	end 
);
