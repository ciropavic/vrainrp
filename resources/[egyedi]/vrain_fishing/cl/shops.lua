...
function fishshop()
	...
	local elements = {}
	table.insert(elements, {label = 'Horgászbot vásárlása - <span style="color:green;">200$</span>', value = 'buyrod'})
	...
	table.insert(elements, {label = 'Kilépés', value = 'exit'})
    ESX.UI.Menu.Open(
...
Citizen.CreateThread(function()
	...
	local w = 1000
        if dist < 5 then
            Draw3DText2(x, y, z + 0.5, "[~r~E~w~] Bolt megnyitása | [~g~G~w~] Hal eladása")
...
Citizen.CreateThread(function()
	...
	AddTextComponentString('Horgászbolt') -- set blip's "name"
