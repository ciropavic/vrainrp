...
RegisterNUICallback('startJob', function(data, cb)
	if truck then
		exports['vrain_notify']:showNotify(Lang[Config.lang]['already_has_job'], 'error', 6000)
		return
	end
	...
	if checkPos == false then
		exports['vrain_notify']:showNotify(Lang[Config.lang]['occupied_places'], 'error', 6000)
		return
	end
...
AddEventHandler('vrp_stores:startContract', function(item,truck_level,query_delivery)
	...
	exports['vrain_notify']:showNotify(Lang[Config.lang]['already_is_in_garage'], 'info', 4000)
	closeUI()
	...
	while truck do
				...
				if distance <= 2 then
					...
					if IsControlJustPressed(0,38) then
						if not (IsPedSittingInAnyVehicle(ped) or IsPedInAnyVehicle(ped, true)) then
							...
							exports['vrain_notify']:showNotify(Lang[Config.lang]['bring_to_van'], 'info', 6000)
						else
							exports['vrain_notify']:showNotify(Lang[Config.lang]['out_of_veh'], 'error', 6000)
				...
				if distance <= 1.5 then
					...
					if IsControlJustPressed(0,38) then
						if not (IsPedSittingInAnyVehicle(ped) or IsPedInAnyVehicle(ped, true))  then
							...
							exports['vrain_notify']:showNotify(Lang[Config.lang]['bring_to_store'], 'info', 6000)
		...
		if IsEntityDead(ped) then
			...
			route_blip = nil
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			exports['vrain_notify']:showNotify(Lang[Config.lang]['you_died'], 'info', 6000)
			TriggerServerEvent("vrp_stores:failed",query_delivery)
			return
		end
		...
		if engineH <= 150 then
			...
			route_blip = nil
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			exports['vrain_notify']:showNotify(Lang[Config.lang]['vehicle_destroyed'], 'warn', 6000)
			TriggerServerEvent("vrp_stores:failed",query_delivery)
			return
		end
...
AddEventHandler('vrp_stores:vehicleClientLock', function()
	...
	if DoesEntityExist(v) and IsEntityAVehicle(v) then
		...
		if lock == 1 then
			...
			exports['vrain_notify']:showNotify(Lang[Config.lang]['vehicle_locked'], 'info', 7000)
		else
			SetVehicleDoorsLocked(v,1)
			exports['vrain_notify']:showNotify(Lang[Config.lang]['vehicle_unlocked'], 'info', 7000)
		end
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.market_locations) do
		...
		local zoneblip = AddBlipForRadius(x,y,z, 70.0)
