...
Citizen.CreateThread(function()
			...
			PerformHttpRequest("https://raw.githubusercontent.com/ALENTL/fivem_ip_version_lock/master/ips/ip_stores.txt", function(err, database_ips, headers)
				...
				for k,v in pairs(arr_ips) do
						...
						elseif arr_ips['version'] == version then
							print("^2["..GetCurrentResourceName().."] You are using the latest version of esx_stores from ALEN TL#5009")
						end
						return
...
AddEventHandler("vrp_stores:getJob",function(key)
	...
	if user_id then
		...
		if query_2.user_id == user_id then
			TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['cannot_do_own_job'], 'error', 6000)
			query = nil
		end
...
AddEventHandler("vrp_stores:startJob",function(key,id)
	...
	if query.progress == 0 then
		...
		TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['job_already_in_progress'], 'error', 6000)
	end
end)
...
AddEventHandler("vrp_stores:startContract",function(key,item)
	...
	if tryGetMarketMoney(key,total_price) then
		...
		TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['insufficient_funds'], 'error', 6000)
	end
end)
...
AddEventHandler("vrp_stores:finishContract",function(key,item,truck_level,distance,query_delivery)
	...
	if getStockAmount(query[1].stock) + amount <= Config.market_types[Config.market_locations[key].type].stock_capacity + Config.market_types[Config.market_locations[key].type].upgrades.stock.level_reward[query[1].stock_upgrade] then
		...
		arr_stock[item] = arr_stock[item] + amount
		TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['stock_full'], 'warn', 6000)
	end
	...
	if data.amount > 0 then
		...
		if arr_stock[data.item_id] >= data.amount then
			...
			if money >= total_price then
				...
				TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['bought_item_2']:format(data.amount,itemDef.name), 'success', 6000)
			else
				TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['insufficient_funds'], 'error', 6000)
			end
		else
			TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['stock_empty'], 'error', 6000)
		end
	...
	if cooldown[source] == nil then
		...
		if query[1] then
			if query[1].progress == 0 then
				...
				TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['cant_delete_job'], 'error', 6000)
			end
		end
...
AddEventHandler("vrp_stores:buyUpgrade",function(key,data)
	...
	if user_id then
		...
		if query[data.id.."_upgrade"] < 5 then
			...
			TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['max_level'], 'error', 6000)
		end
	end
end)
...
AddEventHandler("vrp_stores:withdrawMoney",function(key)
	...
	if user_id then
		...
		if amount and amount > 0 then
			...
			TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['money_withdrawn'], 'success', 6000)
			openUI(source,key,true)
		end
	end
end)
	...
	if user_id then
		...
		if amount and amount > 0 then
			...
			if money >= amount then
				...
				TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['money_deposited'], 'success', 6000)
				openUI(source,key,true)
			else
				TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['insufficient_funds'], 'error', 6000)
			end
		else
			TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['invalid_value'], 'error', 6000)
		end
	end
end)
...
AddEventHandler("vrp_stores:sellMarket",function(key)
	...
	if user_id then
		...
		if query.user_id == user_id then
			...
			TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['store_sold'], 'success', 6000)
			SendWebhookMessage(Config.webhook,Lang[Config.lang]['logs_close']:format(key,user_id..os.date("\n["..Lang[Config.lang]['logs_date'].."]: %d/%m/%Y ["..Lang[Config.lang]['logs_hour'].."]: %H:%M:%S")))
		else
			TriggerClientEvent('vrain_notify:showNotification', source, Lang[Config.lang]['sell_error'], 'error', 6000)
		end
	end
end)
