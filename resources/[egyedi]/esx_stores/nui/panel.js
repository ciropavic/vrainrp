	...
	if (item.showmenu){
		...
		if(item.isMarket == true){
			...
			$('#nav-bar').append(`
				<li id="sidebar-1" onclick="openPage(0)">
					...
					<span class="tooltip">Statisztikák</span>
				</li>
				<li onclick="openPage(4)">
					<i class="fas fa-cash-register"></i>
					<span class="tooltip">Pénztár</span>
				</li>
				<li href="#myModal" data-toggle="modal" >
					<i class="fas fa-times"></i>
					<span class="tooltip">Bolt eladása</span>
				</li>
			`);
		}
		...
		for (const key in config.market_types.market_items) {
			...
			$('#market-products'+item.page).append(`
				<div class="col-6">
					<div class="card overflow-hidden w-auto" style="background:#4e4e4eb4">
						<div class="card-content">
							<div class="card-body cleartfix">
								<div class="media align-items-stretch">
									<div class="align-self-center">
									...
									<div class="media-body">
										...
										<input id="input-` + fkey + `" class="deposit-money pr-0 pl-0 col-6" type="number" min="1" max="` + arr_stock[key] + `" placeholder="      Mennyiség" name="amount" required="required">
										<button onclick="buyItem('` + fkey + `')" class="btn btn-dark">Vásárlás</button>
									</div>
									<div class="align-self-center">
										<h1 style="color:white">` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(item.price_to_customer)  + `</h1>
									</div>
								</div>
							</div>
						</div>
