...
if webhooksFile == nil then
    print('^5[vrain_logs] ^1Error: Could not load webhooks file. Make sure you didn\'t make a typo.^0')
...
function GetPlayerDetails(src, config, channel)
    ...
    if GetConvar("steam_webApiKey", "false") ~= 'false' then
        ...
        _steamID = ""
        _steamURL = ""
        TriggerEvent('Prefech:vrain_logs:errorLog', 'You need to set a steam api key in your server.cfg for the steam identifiers to work!.')
    end
    ...
    if config.Session or config.PlayTime and not webhooksFile[channel].Hide['PlayTime'] then
        if GetResourceState('Prefech_playTime') == "started" then
            ...
            TriggerEvent('Prefech:vrain_logs:errorLog', 'Prefech_playTime is not installed.')
            _total = ""
            _session = ""
        end
    else
...
function CreateLog(args)
    ...
    if webhooksFile[args.channel] then
        ...
        print("vrain_logs: Webhooks channel not found. ("..args.channel..")")
    end
end
...
function getStatus(status, channel)
	if status == 404 or status == 401 and webhooksFile[channel].webhook ~= "DISCORD_WEBHOOK" and webhooksFile[channel].webhook ~= "" then
		print('^3Warn: vrain_logs webhook. Possible invalid webhook for "'..channel..'" webhook. Status code: '..status)
	end
end
