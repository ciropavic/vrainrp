--[[
    ...
    #                                                                   #
    #                 vrain_logs By Prefech 01-11-2021                     #
    #                         www.prefech.com                           #
    #                                                                   #
    #####################################################################
]]
...
if lang == nil then
    print('^5[vrain_logs] ^1Code: Could not load language file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end

if cfgFile == nil then
    print('^5[vrain_logs] ^1Code: Could not load config file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end

RegisterNetEvent('Prefech:vrain_logs:Debug')
AddEventHandler('Prefech:vrain_logs:Debug', log)

RegisterNetEvent('Prefech:vrain_logs:errorLog')
AddEventHandler('Prefech:vrain_logs:errorLog', errorLog)
...
RegisterCommand('jdlogs', function(source, args, RawCommand) --[[Console command to enable debug and error logs.]]
	if source == 0 then
		...
		if args[1]:lower() == "debuglog" then
			SetResourceKvp('vrain_logs:DebugLog', state)
			print('^5[vrain_logs]^1: Updated the debug log status to: '..state)
		elseif args[1]:lower() == "errorlog" then
			SetResourceKvp('vrain_logs:ErrorLog', state)
			print('^5[vrain_logs]^1: Updated the error log status to: '..state)
		end
	end
end)

function debugLog(x)
	if GetResourceKvpString('vrain_logs:DebugLog') == 'true' then
		print("^5[vrain_logs]^0: " .. x)
	end
end

function errorLog(x)
	if GetResourceKvpString('vrain_logs:ErrorLog') == 'true' or GetResourceKvpString('vrain_logs:ErrorLog') == nil then
		print("^5[vrain_logs]^1: " .. x .."^0")
	end
end
...
exports('createLog', function(args)
	if args.screenshot then
		if GetResourceState('screenshot-basic2') == "started" then
			...
			errorLog('Code: SB1000 (You need to have screenshot-basic2 to use screenshot logs.)')
		end
	else
		ServerFunc.CreateLog(args)
	end
...
AddEventHandler("ACCheatAlert", function(args)
	...
	if args.screenshot and GetResourceState('screenshot-basic2') == "started" then
...
AddEventHandler('Prefech:ClientDiscord', function(args)
	if args.screenshot or args.channel == "AntiCheat" then
		if GetResourceState('screenshot-basic2') == "started" then
			...
			errorLog('Code: SB1001 (You need to have screenshot-basic2 to use screenshot logs.)')
			ServerFunc.CreateLog(args)
		end
	else
	...
	if source == 0 then
		if args[1] and has_val(GetPlayers(), args[1]) then
			if GetResourceState('screenshot-basic2') == "started" then
				...
				errorLog('Code: SB2002 (You need to have screenshot-basic2 to use screenshot logs.)')
			end
		end
	elseif IsPlayerAceAllowed(source, cfgFile.screenshotPerms) then
		if args[1] and has_val(GetPlayers(), args[1]) then
			if GetResourceState('screenshot-basic2') == "started" then
				...
				errorLog('Code: SB1002 (You need to have screenshot-basic2 to use screenshot logs.)')
			end
		else
...
CreateThread(function()
	PerformHttpRequest('https://raw.githubusercontent.com/Prefech/vrain_logs/master/json/version.json', function(code, res, headers)
		if code == 200 then
			local rv = json.decode(res)
			if rv.version ~= GetResourceMetadata(GetCurrentResourceName(), 'version') then
				print('^5[vrain_logs] ^1Error: vrain_logs is outdated and you will no longer get support for this version.^0')
			end
		end
	end, 'GET')
end)
...
if GetCurrentResourceName() ~= "vrain_logs" then
    errorLog('This recource should be named "vrain_logs" for the exports to work properly.')
end
...
CreateThread( function()
	...
	SetConvarServerInfo("vrain_logs", "V"..version)
	PerformHttpRequest('https://raw.githubusercontent.com/Prefech/vrain_logs/master/json/version.json', function(code, res, headers)
		if code == 200 then
			local rv = json.decode(res)
...
-------------------------------------------------------^0]]):format(rv.version, rv.changelog))
				if cfgFile['DiscordUpdateNotify'] then
					ServerFunc.CreateLog({ description = "**vrain_logs Update V"..rv.version.."**\nDownload the latest update of vrain_logs here:\nhttps://github.com/prefech/vrain_logs/releases/latest\n\n**Changelog:**\n"..rv.changelog, ping = false, channel = 'system'})
				end
			end
		else
			errorLog('vrain_logs unable to check version')
		end
	end, 'GET')
end)
