...
AddEventHandler('angelicxs-flipcar:flipcar', function()
    ...
    elseif hasRequiredJob() then
        ...
        if dist <= 3 then
            ...
            exports['vrain_notify']:showNotify(Config.Lang['flipped'], 'success', 8000)
            ClearPedTasks(ped)
        else
            exports['vrain_notify']:showNotify(Config.Lang['far_away'], 'error', 8000)
        end
    else
        exports['vrain_notify']:showNotify(Config.Lang['not_allowed'], 'error', 8000)
    end
end)
