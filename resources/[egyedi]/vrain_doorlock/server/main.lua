if not ESX then
	...
	function IsAuthorized(xPlayer, doorID, usedLockpick)
		...
		if Config.AdminAccess and IsPlayerAceAllowed(xPlayer.source, 'command.adminopendoor') and not doorID.ignoreAdmin then
	...
	exports.vAdmin:addCommand(
		...
		function(player)
			TriggerClientEvent("nui_doorlock:newDoorSetup", player.__netId, {});
		end 
	);
	...
	AddEventHandler('nui_doorlock:newDoorCreate', function(config, model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, doubleDoor, doorname)
		...
		if ((exports.vAdmin:getPlayerAdminLevel(source) or 0) < 6) then print(xPlayer.name.. 'attempted to create a new door but does not have permission') return end
