...
AddEventHandler('esx_advancedholdup:robberyCanceled', function(zone)
	...
	if robberyPlayers[source] then
		...
		TriggerClientEvent('vrain_notify:showNotification', xPlayer.source, _U('rob_cancel'), 'warn', 10000)
	end
end)
	...
	if Config.Zones[mainZone] then
		...
		if isCurrentRobbery == false then
			if zone.Robbed ~= 0 and (os.time() - zone.Robbed) < zone.TimeBeforeNewRob then
				...
				TriggerClientEvent('vrain_notify:showNotification', xPlayer.source, _U('rob_already', timerNewRob), 'error', 10000)
				return
			else
				if #xPlayers < zone.PoliceRequired then
					TriggerClientEvent('vrain_notify:showNotification', xPlayer.source, _U('rob_required', zone.PoliceRequired), 'error', 10000)
					return
				end
			end
			...
			for _, xPlayer in pairs(xPlayers) do
				TriggerClientEvent('esx_advancedholdup:robPoliceNotif', xPlayer.source, mainZone, Config.Zones[mainZone].Coords)
				TriggerClientEvent('esx_advancedholdup:createBlip', xPlayer.source, Config.Zones[mainZone].Coords)
			end

			TriggerClientEvent('vrain_notify:showNotification', xPlayer.source, _U('rob_started'), 'info', 10000)
			...
			TriggerClientEvent('vrain_notify:showNotification', xPlayer.source, _U('rob_in_prog'), 'error', 10000)
