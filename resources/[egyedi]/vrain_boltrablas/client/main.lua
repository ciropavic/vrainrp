...
AddEventHandler('esx_advancedholdup:robPoliceNotif', function(zone, coords)
	Citizen.Wait(1)
	local data = {displayCode = '211', description = 'Boltrablás elkezdve', isImportant = 0, recipientList = {'police', 'sheriff', 'deltaforce'}, length = '10000', infoM = 'fa-info-circle', info = zone}
	local dispatchData = {dispatchData = data, caller = 'Riasztó', coords = coords}
...
AddEventHandler('esx_advancedholdup:robCompAtNotif', function(zone, complete)
	...
	if complete then
		local data = {displayCode = '211', description = 'Boltrablás végrehajtva', isImportant = 0, recipientList = {'police', 'sheriff', 'deltaforce'}, length = '10000', infoM = 'fa-info-circle', info = zone}
		local dispatchData = {dispatchData = data, caller = 'Riasztó', coords = Config.Zones[zone].Coords}
		TriggerServerEvent('wf-alerts:svNotify', dispatchData)
	else
		local data = {displayCode = '211', description = 'Boltrablás nem sikerült', isImportant = 0, recipientList = {'police', 'sheriff', 'deltaforce'}, length = '10000', infoM = 'fa-info-circle', info = zone}
		local dispatchData = {dispatchData = data, caller = 'Riasztó', coords = Config.Zones[zone].Coords}
		TriggerServerEvent('wf-alerts:svNotify', dispatchData)
	end
end)
