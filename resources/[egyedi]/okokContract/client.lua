...
AddEventHandler('okokContract:GetVehicleInfo', function(source_playername, date, description, price, source)
	...
	if closestPlayer ~= -1 and playerDistance <= 3.0 then
		...
		if DoesEntityExist(vehicle) and (vehDistance <= 3) then
			...
			exports['vrain_notify']:showNotify('Egy jármű közelében kell lenned, hogy eltudd adni a járműved.', 'error', 10000)
		end
	else
		ClearPedTasks(PlayerPedId())
		exports['vrain_notify']:showNotify('Egy játékos közelében kell lenned, hogy eltudd adni a járműved.', 'error', 10000)
	end
end)
