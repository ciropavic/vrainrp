...
function ToggleVehicleLockSheriffFaction()
	local player = GetPlayerPed(-1)
    local coords = GetEntityCoords(player)
	local vehicle
...
function ToggleVehicleLockMedellinFaction()
	local player = GetPlayerPed(-1)
    local coords = GetEntityCoords(player)
	local vehicle
	...
	local lockStatus = GetVehicleDoorLockStatus(vehicle)
	if PlayerData.job ~= nil and PlayerData.job.name == 'medellin' then
		if lockStatus == 1 then -- unlocked
			lockAnimation()
			SetVehicleLights(vehicle, 2)
			Wait(250)
...
function ToggleVehicleLockGentileFaction()
	local player = GetPlayerPed(-1)
    local coords = GetEntityCoords(player)
	local vehicle
	...
	local lockStatus = GetVehicleDoorLockStatus(vehicle)
	if PlayerData.job ~= nil and PlayerData.job.name == 'gentile' then
		if lockStatus == 1 then -- unlocked
			lockAnimation()
			SetVehicleLights(vehicle, 2)
			Wait(250)
...
function ToggleVehicleLockDeltaFaction()
	local player = GetPlayerPed(-1)
    local coords = GetEntityCoords(player)
	local vehicle
	...
	local lockStatus = GetVehicleDoorLockStatus(vehicle)
	if PlayerData.job ~= nil and PlayerData.job.name == 'deltaforce' then
		if lockStatus == 1 then -- unlocked
			lockAnimation()
			SetVehicleLights(vehicle, 2)
			Wait(250)
	...
	while true do
		...
		if IsControlJustReleased(0, 182) and IsInputDisabled(0) then
			...
			elseif vehicleplate == 'PEARLS' then
				...
				Wait(400)
			elseif vehicleplate == 'SHERIFF' then
				ToggleVehicleLockSheriffFaction()
				Wait(400)
			elseif vehicleplate == 'MEDELLIN' then
				ToggleVehicleLockMedellinFaction()
				Wait(400)
			elseif vehicleplate == 'GENTILE' then
				ToggleVehicleLockGentileFaction()
				Wait(400)
			elseif vehicleplate == 'DELTA' then
				ToggleVehicleLockDeltaFaction()
				Wait(400)
			else
				ToggleVehicleLock()
				Wait(400)
