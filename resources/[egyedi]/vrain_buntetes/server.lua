...
local Webhook = 'https://discord.com/api/webhooks/956248302603624488/_hbtGCKvV_2T2vi78mzLWxqj4o8fwKqDORGuOzGnHWrUozlZ5loXqQ2qerNPipiw9MQ9'
...
AddEventHandler("okokBilling:PayInvoice", function(invoice_id)
	...
	MySQL.Async.fetchAll('SELECT * FROM okokBilling WHERE id = @id', {
		...
		if playerMoney < invoices.invoice_value then
			TriggerClientEvent('vrain_notify:showNotification', xPlayer.source, 'Nincsen elegendő pénzed!', 'error', 10000)
		else
			...
			TriggerClientEvent('vrain_notify:showNotification', xPlayer.source, 'Sikeresen kifizetve!', 'success', 10000)
...
AddEventHandler("okokBilling:CancelInvoice", function(invoice_id)
	...
	MySQL.Async.fetchAll('SELECT * FROM okokBilling WHERE id = @id', {
		...
		TriggerClientEvent('vrain_notify:showNotification', xPlayer.source, 'Sikeresen visszavontad a számlát.', 'info', 10000)
		if Webhook ~= '' then
...
AddEventHandler("okokBilling:CreateInvoice", function(data)
	...
	if Config.LimitDate then
		MySQL.Async.insert('INSERT INTO okokBilling (receiver_identifier, receiver_name, author_identifier, author_name, society, society_name, item, invoice_value, status, notes, sent_date, limit_pay_date) VALUES (@receiver_identifier, @receiver_name, @author_identifier, @author_name, @society, @society_name, @item, @invoice_value, @status, @notes, CURRENT_TIMESTAMP(), DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL @limit_pay_date DAY))', {
			...
			TriggerClientEvent('vrain_notify:showNotification', target.source, 'Kaptál egy új számlát.', 'info', 10000)
			if Webhook ~= '' then
	...
	local information = {
		{
			...
			["author"] = {
				...
				["name"] = Config.ServerName..' - Számla log',
			},
			["title"] = 'Számla #'..data.id..' kifizetve',
			["description"] = '**Címzett:** '..data.player_name..'\n**Érték:** '..data.value..'$\n**Tárgy:** '..data.item..'\n**Frakció:** '..data.society,
	...
	local information = {
		{
			...
			["title"] = 'Számla #'..data.id..' visszavonva',
			["description"] = '**Visszavonva általa:** '..data.name..'\n\n**Címzett:** '..data.player_name..'\n**Érték:** '..data.value..'$\n**Tárgy:** '..data.item..'\n**Frakció:** '..data.society,
	...
	local information = {
		{
			...
			["title"] = 'Számla #'..data.id..' készítve',
			["description"] = '**Általa:** '..data.name..'\n**Frakció:** '..data.society..'\n\n**Címzett:** '..data.player_name..'\n**Érték:** '..data.value..'$\n**Tárgy:** '..data.item,
	...
	local information = {
		{
			...
			["title"] = 'Számla #'..data.id..' automatikusan kifizetve',
			["description"] = '**Címzett:** '..data.player_name..'\n**Érték:** '..data.value..'$\n**Tárgy:** '..data.item..'\n**Frakció:** '..data.society,
