...
window.onload = function () {
  var eventCallback = {
    ui: function (data) {
      ...
      if (config.showVoice == true) {
        ...
        document.querySelector(".info.time-and-place").style.right = "25px";
      }
    ...
    setText: function (data) {
      const elem = document.querySelector(`#date span`);
      elem.innerHTML = `Dátum: <strong>${moment().locale("hu").format("LL")} ${moment()
        .locale("hu")
        .format("LTS")}</strong>`;
      ...
      if (data.isdead == true) {
        if (document.querySelector("#health").classList.contains("dead") == false) {
          ...
          for (i = 0; i < data.status.length; i++) {
            document.querySelector("#" + data.status[i].name + " span").style.height = "0";
          }
        }
      } else {
        for (i = 0; i < data.status.length; i++) {
          ...
          if (statusValue <= 35) {
            ...
            if (document.querySelector("#" + data.status[i].name)) {
              document.querySelector("#" + data.status[i].name).classList.remove("dying");
            }
          }
        }
      ...
      if (data.status == true) {
        if (vehicleInfo.classList.contains("inactive")) {
          ...
          if (vehiclesCars.indexOf(data.type) > -1) {
            ...
            if (data.type == 13) {
              document.querySelector("#vehicle-others i").classList.add("fa-biking");
            } else if (data.type == 14) {
              document.querySelector("#vehicle-others i").classList.add("fa-ship");
            } else if (data.type == 15) {
              document.querySelector("#vehicle-others i").classList.add("fa-helicopter");
            } else if (data.type == 16) {
              document.querySelector("#vehicle-others i").classList.add("fa-plane");
            } else if (data.type == 21) {
              document.querySelector("#vehicle-others i").classList.add("fa-train");
            }
          }
  ...
  document
    ...
    .addMultiListener("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", function () {
