...
$(document).on('click', "#submitFeedback", function() {
	...
	$.post('https://vrain_reportsystem/action', JSON.stringify({
		action: "newFeedback",
		subject: subject,
...
function reloadFeedbackTable() {
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					<td>JELENTÉS #${feedback.feedbackid}</th>
					<td>${category}</td>
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("SZÉP MUNKA! MOST PÁR PILLANATRA LEPIHENHETSZ.");
		...
		$(".feedback_subtitle").html("ÚGY LÁTSZIK MUNKÁD VAN...");
