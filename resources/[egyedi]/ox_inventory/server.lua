		...
		if durability then
			if durability > 100 then
				if os.time() > durability then
					...
					TriggerClientEvent('ox_lib:notify', source, { type = 'error', position = "top", description = shared.locale('no_durability', data.label) })
					return
				end
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			elseif server?.UsableItemsCallbacks[item.name] then
				...
				if item.consume and data.count >= item.consume then
					...
					TriggerClientEvent('ox_lib:notify', source, { type = 'error', position = "top", description = shared.locale('item_not_enough', item.name) })
				end
			end
		end
	end
