    ...
    methods: {
        ...
        OffenderSearch() {
            if (this.offender_search) {
                ...
                $.post('http://vrain_mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://vrain_mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://vrain_mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://vrain_mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && this.report_new.incident) {
                $.post('http://vrain_mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://vrain_mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://vrain_mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://vrain_mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://vrain_mdt/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://vrain_mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://vrain_mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://vrain_mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://vrain_mdt/getReport', JSON.stringify({
                id: id
            }));
            this.modal = 'loading';
            return;
        ...
        AttachToCall(index) {
            $.post('http://vrain_mdt/attachToCall', JSON.stringify({
                index: index,
        ...
        DetachFromCall(index) {
            $.post('http://vrain_mdt/detachFromCall', JSON.stringify({
                index: index
            }));
            this.current_call = {
                source: null,
        ...
        SetCallWaypoint(index) {
            $.post('http://vrain_mdt/setCallWaypoint', JSON.stringify({
                coords: this.calls[index].coords
            }));
            return;
        },
        ...
        SaveEditCall() {
            $.post('http://vrain_mdt/editCall', JSON.stringify({
        ...
        DeleteCall(index) {
            $.post('http://vrain_mdt/deleteCall', JSON.stringify({
                index: index
            }));
            return;
        },
        ...
        SaveVehicleEditChanges() {
            $.post('http://vrain_mdt/saveVehicleChanges', JSON.stringify({
        ...
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                if (mdtApp.current_call.id == mdtApp.calls[event.data.call].id) {
                    $.post('http://vrain_mdt/deleteCallBlip')
                    mdtApp.current_call = {
                        source: null,
                        details: null,
                        id: null,
