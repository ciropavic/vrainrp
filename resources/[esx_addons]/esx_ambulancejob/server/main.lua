...
ESX.RegisterServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function(source, cb)
	...
	if Config.OxInventory and Config.RemoveItemsAfterRPDeath then
		...
		for item, data in pairs(exports.ox_inventory:Items()) do
			...
			if (itemCount and itemCount > 0) then 
				if (item == "money") then 
					exports.ox_inventory:RemoveItem(source, "money", itemCount * Config.MoneyTakenPercent);
				elseif (not Config.IgnoredItemsWhenRemove[item]) then 
	...
	if Config.RemoveCashAfterRPDeath then
		if xPlayer.getMoney() > 0 then
			print("take", math.ceil(xPlayer.getMoney() * Config.MoneyTakenPercent));
			xPlayer.removeMoney(math.ceil(xPlayer.getMoney() * Config.MoneyTakenPercent));
		end
