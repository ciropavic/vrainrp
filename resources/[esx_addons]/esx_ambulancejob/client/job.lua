...
function OpenAmbulanceActionsMenu()
	...
	if Config.EnablePlayerManagement and ESX.GetPlayerData().job.grade_name == 'boss' or ESX.GetPlayerData().job.grade_name == 'igazgato' then
	...
	while true do
		...
		if ESX.GetPlayerData().job and ESX.GetPlayerData().job.name == 'ambulance' then
			...
			for hospitalNum,hospital in pairs(Config.Hospitals) do
				...
				--Armoury/Storage
				for k,v in ipairs(hospital.Armoury) do
					local distance = #(playerCoords - v)
					...
					if distance < Config.DrawDistance then
						...
						if distance < Config.Marker.x then
							isInMarker, currentHospital, currentPart, currentPartNum = true, hospitalNum, 'Armoury', k
						end
					end
				end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'Pharmacy' then
					OpenPharmacyMenu()
				elseif CurrentAction == 'Armoury' then
					OpenArmoryMenu()
...
function OpenArmoryMenu()
	if Config.OxInventory then
		exports.ox_inventory:openInventory('stash', {id = 'society_ambulance', owner = station})
