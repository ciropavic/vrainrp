  ...
  Citizen.CreateThread(function()
	...
	while true do
		...
		if (IsControlJustPressed(1, 101)) then
			local coords = GetEntityCoords(GetPlayerPed(-1))
			local vehicle = GetClosestVehicle(coords, 7)
			local lockstatus = GetVehicleDoorLockStatus(vehicle)
			local info = ESX.Game.GetVehicleProperties(vehicle)
			local plate = info.plate

			print(plate)
			TriggerServerEvent('project:getvehicleinfos', plate, vehicle)
	  	end
	end
end)

RegisterNetEvent('project:opencar')
AddEventHandler('project:opencar', function(vehicle)
	local coords = GetEntityCoords(GetPlayerPed(-1))
	local lockstatus = GetVehicleDoorLockStatus(vehicle)
	local info = ESX.Game.GetVehicleProperties(vehicle)
	local plate = info.plate
	...
	if lockstatus == 0 or lockstatus == 1 then
		...
		ESX.ShowNotification("DEBUG - Nyitva")
	...
	elseif lockstatus == 2 then
		...
		ESX.ShowNotification('DEBUG - Zárva')
