...
function RemoveGear(weapon)
	...
	for i, entity in pairs(HideList) do
		if entity.weapon ~= weapon then
			_HideList[i] = entity
		else
			DeleteWeapon(entity.obj)
...
function SetGear(weapon)
	if (IsHidden) then 
		table.insert(HideList, { weapon = weapon, obj = -1 });
		return;
	end 
...
-- 
-- Added fetches to disable
-- 

exports.vCore:addFetch(
	"realweapons::addWeapon",
	function(callback, weaponId)
		print("ujfegyo", weaponId);
		if (weaponId) then 
			SetGear(weaponId);
			callback(true)
		end 
	...
	function(callback)
		...
		if (HideList) then 
			for i,v in pairs(HideList) do 
				...
				for i=1, #Config.RealWeapons, 1 do
					if Config.RealWeapons[i].name == v.weapon then
						bone     = Config.RealWeapons[i].bone
						boneX    = Config.RealWeapons[i].x
						boneY    = Config.RealWeapons[i].y
						boneZ    = Config.RealWeapons[i].z
				...
				SpawnObject(model, {
					...
					local bonePos 	= GetWorldPositionOfEntityBone(playerPed, boneIndex)
					AttachEntityToEntity(obj, playerPed, boneIndex, boneX, boneY, boneZ, boneXRot, boneYRot, boneZRot, false, false, false, false, 2, true)
					table.insert(Weapons,{weapon = v.weapon, obj = obj})
				end)
			end
		end 
