...
function openShop(shopId)
    ESX.TriggerServerCallback('skVehicleshop:getShopVehicles', function(response)
        local data = json.decode(response);
        if (data[1] and data[1].model) then 
            DisplayRadar(false);
...
-- Sima kereskedés marker 
AddEventHandler(config.resource..':onMarkerHit', function(id, entity, isLocalPlayer, data)
    local id = data.id;
    shopId = id;

    if (data and data.type and data.type == 'shop' and config.shops[id] and isLocalPlayer) then 
        exports.vInteraction:addInteraction("E", "Járműbolt megnyitása", function() openShop(id); end, { interval = 2000 });
    end 
end);
...
RegisterNUICallback('buyvehicle', function(data)
    if (data and data.model) then 
        ESX.TriggerServerCallback('skVehicleshop:buyVehicle', function(success, data)
            if (success) then 
                closeShop();
                ...
                ESX.ShowNotification(data.reason);
            end
        end, shopId, data.model);
    end
end);
