...
Citizen.CreateThread(function()
    ...
    local activeDims = vec2(activeSize.x * (activeRes.x / activeRes.y) * 0.5, activeSize.y * 0.5)
    ...
    while true do
        ...
        local handle = StartExpensiveSynchronousShapeTestLosProbe(r_pos.x,r_pos.y,r_pos.z, b.x,b.y,b.z, 1|2|4|8|16, PlayerPedId(), 7)
        local _,hit,pos,surface,entity = GetShapeTestResult(handle)
        if hit ~= 0 then
            kurvaanyad(entity);

            -- -- Draw a preview using "DrawMarker":
...
function kurvaanyad(num)
    for _, idk in ipairs(GetGamePool("CPed")) do 
        if (num == idk) then 
            print(num, "torles")
            -- SetPedRagdollForceFall(idk);
            DeletePed(idk);
        end 
    end 
end 